package New.maze;

import java.util.Stack;

//对迷宫单元的定义
public class Cell {

    public static void main(String[] args){
        char[][] maze=new char[10][10];
        init(maze);
        for (int i=0;i<maze.length;i++){
            maze[0][i]='1';
            maze[9][i]='1';
            maze[i][0]='1';
            maze[i][9]='1';
        }
        maze[1][3]='1';
        maze[1][4]='1';
        maze[1][5]='1';
        maze[1][8]='1';
        maze[2][3]='1';
        maze[2][4]='1';
        maze[2][7]='1';
        maze[3][7]='1';
        maze[4][5]='1';
        maze[4][6]='1';
        maze[5][3]='1';
        maze[5][4]='1';
        maze[5][5]='1';
        maze[6][5]='1';
        maze[6][7]='1';
        maze[7][2]='1';
        maze[7][3]='1';
        maze[7][7]='1';
        maze[8][1]='1';
        maze[8][6]='1';


        mazeExit(maze,1,1,2,1);
    }

    int x=0; //单元所在行
    int y=0; //单元所在列
    boolean visited=false; //是否访问过
    char c=' '; //是墙('1'),可通路('0') 或起点到终点的路径('*')
    public Cell(int x,int y,char c,boolean visited){
        this.x=x;
        this.y=y;
        this.c=c;
        this.visited=visited;
    }

    /**
     *
     * @param maze 目标迷宫
     * @param sx    起始点的横坐标
     * @param sy    起始点的纵坐标
     * @param ex    终点的横坐标
     * @param ey    终点的纵坐标
     */
    public static void mazeExit(char[][] maze,int sx,int sy,int ex,int ey){
        Cell[][] cells=createMaze(maze); //创建迷宫
        printMaze(cells);   //打印迷宫
        Stack<Cell> s=new Stack();
        Cell startCell=cells[sx][sy]; //起点
        Cell endCell=cells[ex][ey]; //终点
        s.push(startCell); //起点入栈
        startCell.visited=true; //标记起点已访问

        while (!s.isEmpty()){
            Cell current=s.peek();
            if (current==endCell){ //路径找到
                while (!s.isEmpty()){
                    Cell cell=s.pop(); //沿路返回将路径上的单元设为*
                    cell.c='*';
                    //堆栈中与cell相邻的单元才是路径的组成部分，除此之外
                    //堆栈中还有记录下来但是未继续向下探索的单元，这些单元直接出栈
                    //这里有点不好理解
                    while (!s.isEmpty() && ! isAdjoinCell(s.peek(),cell)) s.pop();
                }
                System.out.println("找到从起点到终点的路径。");
                printMaze(cells);
                return;
            }else { //如果当前位置不是终点
                int x=current.x;
                int y=current.y;
                int count=0;

                if (isVaildWayCell(cells[x+1][y])) { //向下
                    s.push(cells[x + 1][y]);
                    cells[x + 1][y].visited = true;
                    count++;
                } if (isVaildWayCell(cells[x][y+1])) { //向右
                    s.push(cells[x ][y+1]);
                    cells[x][y+1].visited = true;
                    count++;
                } if (isVaildWayCell(cells[x-1][y])) { //向上
                    s.push(cells[x - 1][y]);
                    cells[x - 1][y].visited = true;
                    count++;
                } if (isVaildWayCell(cells[x][y-1])) { //向左
                    s.push(cells[x ][y- 1]);
                    cells[x ][y- 1].visited = true;
                    count++;
                }
                if (count==0) s.pop(); //如果是死点，出栈
            }//end of if
        }//end of while
    }

    private static void printMaze(Cell[][] cells){
        for (int x=0;x<cells.length;x++){
            for (int y=0;y<cells[x].length;y++){
                System.out.print(cells[x][y].c+" ");
            }
            System.out.println();
        }
    }

    //判断两个cell是否相邻
    private static boolean isAdjoinCell(Cell cell1,Cell cell2){
        if (cell1.x==cell2.x && Math.abs(cell1.y-cell2.y)<2) return true;
        if (cell1.y==cell2.y && Math.abs(cell1.x-cell2.x)<2) return true;
        return false;
    }

    //判断一个cell是否可以走
    private static boolean isVaildWayCell(Cell cell){
        return cell.c=='0' && !cell.visited;
    }


    //传入一个二维的char数组，返回一个 二维的Cell数组
    private static Cell[][] createMaze(char[][] maze){
        Cell[][] cells=new Cell[maze.length][];
        for (int x=0;x<maze.length;x++){
            char[] row=maze[x];
            cells[x]=new Cell[row.length];
            for (int y=0;y<row.length;y++){
                cells[x][y]=new Cell(x,y,maze[x][y],false);
            }
        }
        return cells;
    }

    public static void init(char[][] chars){
        for (int i=0;i<chars.length;i++){
            for (int j=0;j<chars[i].length;j++){
                chars[i][j]='0';
            }
        }
    }
}
